...
// Featured products in the specified sequence with CORRECT product names
const featured = [
  // Chemical Division - in specified order
  { product: chemicalProducts.find(p => p.name === "Open Loop Cooling Water Systems") || chemicalProducts[0], division: "chemical" },
  { product: chemicalProducts.find(p => p.name === "Closed Loop Cooling Systems") || chemicalProducts[1], division: "chemical" },
  { product: chemicalProducts.find(p => p.name === "UNI-TREAT® RO Chemicals") || chemicalProducts[4], division: "chemical" },
  { product: chemicalProducts.find(p => p.name === "Boiler Water Treatment for Low, Medium & High Pressure Boilers") || chemicalProducts[2], division: "chemical" },
  { product: chemicalProducts.find(p => p.name === "Descaling Products") || chemicalProducts[3], division: "chemical" },
  ...
  // Mechanical Division
  { product: mechanicalProducts.find(p => p.name === "Multi Grade Sand Filters, Activated Carbon Filters and Iron Removal Filters") || mechanicalProducts[0], division: "mechanical" },
  { product: mechanicalProducts.find(p => p.name === "Water Softening Plant") || mechanicalProducts[1], division: "mechanical" },
  { product: mechanicalProducts.find(p => p.name === "Demineralization Systems (D. M. Plant)") || mechanicalProducts[2], division: "mechanical" },
  { product: mechanicalProducts.find(p => p.name === "Reverse Osmosis System (R. O. Plant)") || mechanicalProducts[3], division: "mechanical" },
  { product: mechanicalProducts.find(p => p.name === "Ultra Violet Dis-Infection System") || mechanicalProducts[5], division: "mechanical" },
  
  // Spares & Equipments
  { product: sparesProducts.find(p => p.name === "Water Testing Kits & Equipments") || sparesProducts[0], division: "spares" },
  { product: sparesProducts.find(p => p.name === "Filtration Cartridges and Housings") || sparesProducts[1], division: "spares" },
  { product: sparesProducts.find(p => p.name === "Water Treatment Plants Spares") || sparesProducts[2], division: "spares" },
];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-12">
          {featured.map(({ product, division }, i) => {
            ...
            if (!product) {
              console.warn(`Product not found for index ${i}`);
              return null;
            }
                ...
                <Link href={`/products/${division}/${product.slug}`} className="block h-full">
                  <div className="relative bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden h-full border border-gray-100">
                    ...
                    <div className="p-6 relative z-10">
                      ...
                      {/* Product Name - simplified for display */}
                      <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-2 group-hover:text-[#1E3A8A] transition-colors duration-300 line-clamp-2">
                        {product.name.split("UNI-TREAT® ").pop().split(" for")[0]}
                      </h3>

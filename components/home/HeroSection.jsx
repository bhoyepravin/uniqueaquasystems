...
export default function HeroSection() {
  ...
  // Only show carousel functionality on homepage
  const isHomePage = pathname === '/';
  ...
  // Setup auto-slide for homepage only
  useEffect(() => {
    if (isHomePage) {
      clearAutoSlide();
  ...
  }, [isHomePage, nextSlide, clearAutoSlide]);
  ...
  // Touch handlers with improved validation
  ...
  // For non-homepage, just show the first slide without carousel functionality
  const displaySlide = isHomePage ? slides[current] : slides[0];

  return (
    <section 
      className="relative w-full min-h-screen overflow-hidden"
      onTouchStart={isHomePage ? handleTouchStart : undefined}
      onTouchMove={isHomePage ? handleTouchMove : undefined}
      onTouchEnd={isHomePage ? handleTouchEnd : undefined}
    >
      {/* Background gradient overlay - Changed to absolute */}
      <div className="absolute inset-0 z-0">
        <div 
          className="absolute inset-0"
          style={{
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={isHomePage ? current : 0}
          initial={{ opacity: isInitialLoad ? 1 : 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          transition={{ duration: isInitialLoad ? 0 : 0.7, ease: "easeInOut" }}
          className="relative z-10 min-h-screen"
        >
          {/* Background image - Changed to absolute */}
          <div
            className="absolute inset-0 bg-cover bg-center"
            style={{
              backgroundImage: `url(${getImageUrl(displaySlide.image)})`,
              backgroundSize: "cover",
              backgroundPosition: "center",
              opacity: 0.5,
            }}
          ...
          {/* Darker overlay for better text contrast - Changed to absolute */}
          <div className="absolute inset-0 bg-gradient-to-b from-black/20 via-transparent to-black/30" />
          ...
          <div className="relative z-10 min-h-screen flex items-center">
            <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
              ...
              <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 xl:gap-16 items-center">
                ...
                {/* Image Container */}
                <div className="order-1 lg:order-2 mb-6 sm:mb-8 lg:mb-0">
                  <div className="relative w-full max-w-[260px] sm:max-w-[320px] md:max-w-[380px] lg:max-w-lg xl:max-w-xl mx-auto">
                    <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/20 to-blue-500/20 rounded-full blur-3xl -z-10 hidden sm:block" />
                    
                    <div className="relative aspect-square w-full">
                      {typeof displaySlide.image === 'string' ? (
                        <img
      ...
      {/* Navigation Arrows - ONLY show on homepage with z-index 20 */}
      {isHomePage && (
        <>
          <button
            onClick={prevSlide}
            className="hidden md:flex absolute left-2 sm:left-4 md:left-6 top-1/2 -translate-y-1/2 backdrop-blur-md p-2 sm:p-2.5 md:p-3.5 rounded-full text-white transition-all duration-300 hover:scale-110 group z-20"
            style={{
              background: "rgba(255,255,255,0.12)",
              border: "1px solid rgba(255,255,255,0.25)",
              backdropFilter: "blur(8px)",
          ...
          <button
            onClick={nextSlide}
            className="hidden md:flex absolute right-2 sm:right-4 md:right-6 top-1/2 -translate-y-1/2 backdrop-blur-md p-2 sm:p-2.5 md:p-3.5 rounded-full text-white transition-all duration-300 hover:scale-110 group z-20"
            style={{
              background: "rgba(255,255,255,0.12)",
              border: "1px solid rgba(255,255,255,0.25)",
              backdropFilter: "blur(8px)",
